...
export function openGuestWindow ({ event, embedder, guest, referrer, disposition, postData, overrideBrowserWindowOptions, windowOpenArgs }: {
  ...
  embedder.emit('did-create-window', window, { url, frameName, parsedOptions: browserWindowOptions, disposition, additionalFeatures, referrer, postData })
...
function getDeprecatedInheritedOptions (embedder: WebContents) {
  const { type, ...inheritableOptions } = (embedder as any).browserWindowOptions
